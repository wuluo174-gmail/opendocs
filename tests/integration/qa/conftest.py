"""Shared fixtures for S5 QA and summary integration tests."""
...
def qa_db(tmp_path: Path) -> Path:
    return tmp_path / "qa_test.db"


@pytest.fixture()
def qa_runtime(qa_db: Path, tmp_path: Path) -> OpenDocsRuntime:
    init_db(qa_db)
    engine = build_sqlite_engine(qa_db)
    hnsw_path = tmp_path / "hnsw" / "qa_test.hnsw"
...
@pytest.fixture()
def indexed_qa_env(qa_corpus: Path, qa_db: Path, qa_runtime: OpenDocsRuntime) -> tuple[Engine, Path, Path]:
    engine = qa_runtime.engine
    hnsw_path = qa_runtime.hnsw_path
    assert hnsw_path is not None

    source = SourceService(engine, runtime=qa_runtime).add_source(qa_corpus)
    qa_runtime.build_index_service().full_index_source(source.source_root_id)
    return engine, qa_db, hnsw_path


@pytest.fixture()
def qa_service(indexed_qa_env: tuple[Engine, Path, Path], qa_runtime: OpenDocsRuntime) -> QAService:
    return qa_runtime.build_qa_service()


@pytest.fixture()
def qa_search_service(
    indexed_qa_env: tuple[Engine, Path, Path],
    qa_runtime: OpenDocsRuntime,
) -> SearchService:
    return qa_runtime.build_search_service()
