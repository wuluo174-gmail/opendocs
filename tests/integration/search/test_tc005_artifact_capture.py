...
class TestTc005ArtifactCapture:
    def test_capture_writes_manifest_query_log_and_screenshots(self, tmp_path: Path) -> None:
        ...
        expected_capture_query_ids = {
            case.query_id for case in load_s4_acceptance_capture_cases().tc005
        }
        ...
        assert any(
            "page=" in artifact.locator_label or "paragraph=" in artifact.locator_label
    ...
    def test_capture_refuses_overwrite_without_force(self, tmp_path: Path) -> None:
        ...
        planned_output = next(
            path for path in planned_tc005_output_paths(output_dir) if path.suffix == ".png"
        )
