...
- `src/opendocs/ui/acceptance_capture.py` — TC-005 / TC-018 验收工件的固定生成逻辑
...
- 两类 manifest 现在都会显式记录输入资产 provenance，而且 provenance policy 本身也收回到 retrieval 层，不再由 UI 工件代码私有；这样 `TC-005/TC-018` 的工件能直接回溯到对应的 stage assets。

### 过滤正确性修复
- 修复了 `time_range` 过滤使用 ISO `T` 格式导致 SQLite 文本比较失真的问题，统一改为 SQLite 兼容时间格式。
- 修复了 dense 检索在 `allowed_ids` 下依赖 hnswlib 原生 filter 可能空结果的问题；现在先取候选再在 Python 侧精确过滤，保证“过滤后仍返回可引用片段”。
...
- **修复**：`TC-005/TC-018` 的截图选择规则不再散落在 `acceptance_capture.py` 代码常量里，改成 `src/opendocs/retrieval/assets/s4_acceptance_capture_cases.json` + 共享加载器；测试、工件脚本和阶段文档现在消费同一个 capture owner。
- **修复**：`TC-005/TC-018` 的 planned outputs 不再手写文件名列表；现在由 capture 资产的 slug 统一派生。`TC-018` 的页码型 / 段落型语义也成为资产字段，不再让测试用硬编码文件名反推。
- **修复**：`TC-005/TC-018` 的默认 acceptance corpus 不再由 `acceptance_capture.py` 自己决定；现在统一改成 `src/opendocs/retrieval/assets/s4_acceptance_corpora.json` + 共享 loader，`TC-005` 的 manifest 标签和 `TC-018` 的 fixture 路径都由同一个 corpus owner 派生。
...
- **修复**：manifest provenance 的 owner 不再是 `acceptance_capture.py` 内部 helper，改成 `src/opendocs/retrieval/stage_acceptance_provenance.py`；集成测试现在对齐 retrieval 层 owner，而不是对齐 UI 实现本身。
- **修复**：S4 runtime synonym lexicon 与 acceptance golden queries 重新锁回同一份阶段批准基线；当前词表资产严格是 `5` 条同义触发词，`TC-005` 仍保持 `5 条定位 + 5 条同义 + 1 条零结果`，loader 重新要求 synonym queries 全覆盖 runtime lexicon，避免未批准词条偷偷进入运行时能力面。
- **修复**：`TC-005` 的 manifest provenance 现在显式包含 runtime 同义词词表资产 `src/opendocs/retrieval/assets/stage_query_lexicon.json`；这样同义查询命中能力的真正输入 owner 也能被工件直接审计，不再只看到 golden queries 而看不到底层 lexicon。
