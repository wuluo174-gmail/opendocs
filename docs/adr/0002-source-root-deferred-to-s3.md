...
当时的主规范数据模型只显式列出 Document、Chunk、KnowledgeItem、RelationEdge、MemoryItem、FileOperationPlan、AuditLog。**SourceRoot（文档源根目录）未被列为 S1 领域对象。**
...
1. **当时的主规范未要求 S1 实现 SourceRoot 实体**：领域对象列表中不含此对象，S1 出口条件也未提及。
