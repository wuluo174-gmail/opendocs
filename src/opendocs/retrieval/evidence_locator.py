...
class EvidenceLocator:
    ...
    def resolve_open_target(
        ...
        target = self._resolve_target(session, doc_id, chunk_id)
        if target is None or not target.can_open:
            return None
        return (
    ...
    def _build_open_target(
        ...
        *,
        ...
        if page_no is not None and path.suffix.lower() == ".pdf":
            return f"{path.resolve().as_uri()}#page={page_no}"
        if paragraph_range is not None or char_range is not None:
            fragments: list[str] = []
            if paragraph_range is not None:
                fragments.append(f"paragraph={paragraph_range}")
            if char_range is not None:
                fragments.append(f"char={char_range}")
            return f"{path.resolve().as_uri()}#{'&'.join(fragments)}"
        return str(path)
    ...
    def open_file(
        ...
        *,
        ...
        context at the UI/service boundary. PDF and non-PDF locator hints are
        passed through as URI fragments on a best-effort basis.
        """
        p = Path(path)
        if not p.exists():
            return False
        target = EvidenceLocator._build_open_target(
            p,
            page_no=page_no,
        ...
        try:
            if system == "darwin":
                ...
                subprocess.Popen(["start", "", target], shell=True)  # noqa: S603 S602
            else:
                return False
            return True
        except OSError:
